...
different OpenFOAM versions w.r.t. features and API (see also ). If
...
Note that this needs to be done sourcing `$FOAM_BASH` to make sure
...
As such, . Instead, you should track down the root cause of the floating
...
cores that are used for the actual simulation, see also .
...
When running OpenFOAM in parallel, , to avoid running the same OpenFOAM
...
see for more information on `mympirun`.

See for how to get started with `mympirun`.

To pass down the environment variables required to run OpenFOAM (which
were defined by the `$FOAM_BASH` script, see ) to each of the MPI
...
::: prompt
number of processor directories = 4 is not equal to the number of
processors = 16
:::
...
See to control the number of process `mympirun` will start.
...
-   instruct OpenFOAM to write out results at a reasonable frequency, ;
    you can control this using the `writeControl`, `writeInterval`,
    etc. keywords;
...
-   if you do not plan to change the parameters of the OpenFOAM
    simulation while it is running, to avoid that OpenFOAM re-reads each
...
`$VSC_SCRATCH` filesystem. , since these shared filesystems are too slow
for these type of workloads.

For large parallel OpenFOAM simulations on the Tier-2 clusters, consider
